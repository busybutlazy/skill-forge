...
skill-forge 已提供的 Change Workflow skills：
...
skill-forge 的 `agent-hooks` guideline item 可為 Claude Code 與 Codex 安裝原生 `PreToolUse` 安全 hooks，阻擋明確的危險命令與受保護路徑寫入。執行環境需要 `python3` 3.11 或更新版本；Codex 仍可能要求使用者信任精確的 hook definition。Hooks 是縱深防禦，不能取代 sandbox、權限、CI 或人工批准。
...
再逐步增加：implement-task skill、reviewer subagent、security review。若目標環境具備 Python 3.11+，可透過 guideline 安裝 protected-file / dangerous-command hooks。
