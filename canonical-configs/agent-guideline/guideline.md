...
這張表只選擇入口，不取代各 skill 的 admission、approval 或 authority 規則。並非所有專案都必須先執行 grilling；決策已完整時直接進入相應下游入口。
...
| Skill | 用途 | 禁止事項 |
|-------|------|----------|
| `grill-with-docs` | 收斂影響 scope、contract、domain、security、data 或 major architecture 的重要決策 | production implementation、自行批准決策 |
| `define-project` | 將已收斂決策整理為可批准的 SPEC、必要 CONTRACTS 與 outcome-based ROADMAP | 猜測未決策答案、自行批准或啟動 bootstrap |
