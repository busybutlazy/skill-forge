...
- 問題尚未解決，解法仍不確定
- 使用者明確說不需要記錄
- 問題是 AI 在實作過程中自己撞到的技術障礙（語言／框架陷阱、套件 API 行為、執行環境、容器、CI、build／deploy 設定等），與使用者實際使用流程無關
...
- **使用者觸發 → 採納**：使用者在訊息中主動提問，或描述自己執行某個流程／指令／系統時遇到的錯誤、非預期結果
- **AI 觸發 → 排除**：AI 自己在執行 tool、跑測試、修 bug、build／deploy 過程中撞到的問題

**優先採納（使用者觀點）：**
- 使用者主動提出的問題、疑問、需求（「為什麼…」「怎麼做…」「我看不到…」）
- 使用者操作工具或系統、執行指令時遇到的錯誤訊息或非預期行為
- 使用者要求的決策、規範、流程說明
- 使用者反覆問過、或可能會再問的問題

**通常排除（AI 在實作過程中遇到的技術障礙）：**
- 任何程式語言或框架的行為陷阱、API 差異、版本差異
- 套件、執行環境、容器、CI、build／deploy 設定
- import 路徑、模組或檔案同步問題
- mock、fixture、測試環境配置
- 為了讓某段程式或指令跑通而臨時調整的環境變數、權限、路徑

**邊界 case**：使用者本人是工程師，且**該問題是使用者本人觸發、自己未來會再遇到** → 仍採納。
此條只是「使用者觸發」的特例，不是繞過上方排除規則的旁路。判斷重點是「**使用者本人會不會再遇到**」，不是「跟技術有沒有關」。

**檢索試紙**：「未來使用者遇到類似情境時，會主動找這張卡片來看嗎？」答案不確定就不要建立。
...
- 一次性問題，幾乎不會再發生
- 對話只是延伸討論，沒有明確結論
- 問題屬於 AI 自行 debug 的過程，使用者不會直接碰到

**若沒有找到任何有價值的片段：** 明確告知使用者「這次對話沒有找到適合沉澱的內容」，並說明原因（例如問題未解決、內容一次性等），不要靜默結束。

從 session 中找出 1-3 個候選片段。若超過 3 個潛在主題，優先選擇：(1) 已有明確解法的、(2) 最有可能重複發生的、(3) 對他人最有參考價值的。
...
|------|---------|
| 無相似卡片 | 直接進入 Step 3 |
| 有相似但內容不足 | 告知使用者既有卡片路徑，詢問是補充既有卡片還是建立新卡片 |
| 幾乎完全重複 | 提示既有卡片路徑，詢問是否要更新而非新建 |

補充既有卡片時，使用 Edit 工具修改對應欄位，並在 `updated_at` 欄位（若有）記錄今天日期。

若 `docs/experience-cards/` 目錄不存在，跳過此步驟直接建立。

### Step 3: Generate Card Draft
...
> RAG pipeline 建議 embed `title + search_hint` 的串接文字，而非只 embed title。
...
what_users_usually_try: >  # 使用者的直覺反應，以及為何這樣做會出問題
...
- 生成的卡片必須能獨立閱讀，不依賴對話上下文（讀者沒看過這次對話也能看懂）
- `title` 必須清楚描述問題或主題，讀者不看內容就能判斷是否相關
- `tags` 至少 2 個，包含主題關鍵字和工具/系統名稱
- `expected_result` 或 `verification` 欄位必須具體到使用者能自行判斷是否成功，不接受「完成」「正常」等模糊描述
- 敏感資訊必須在寫入前完成遮蔽
- 不強制詢問使用者拒絕建立卡片的原因
