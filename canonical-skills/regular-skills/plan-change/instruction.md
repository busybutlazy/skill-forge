...
5. Divide work into independently verifiable tasks. Identify contract, dependency, migration, security, and data-loss approval points.
6. Define normal, boundary, failure, compatibility, and security tests. Verification commands must use existing Docker/Compose/Make/container wrappers only.
7. Write `changes/<change-id>/IMPLEMENTATION_PLAN.md` using [the template](./references/IMPLEMENTATION_PLAN_TEMPLATE.md).
8. Present unresolved decisions and stop for explicit human approval. Do not invoke implementation.
