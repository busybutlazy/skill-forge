...
對目標程式碼進行安全性、邏輯正確性、效能與可維護性的系統性審查，產出結構化繁體中文報告，並提供互動式修復追蹤。
...
每個發現前方的 `- [ ]` 是修復追蹤 checkbox，修復完成後會更新為 `- [x]`。

如果沒有任何發現，仍然產出報告，在摘要註明無發現，並在審查範圍限制說明覆蓋範圍。

### 8. Present findings and offer scheduling assistance

報告寫入檔案後：

1. 在對話中列出所有 CRITICAL 和 HIGH 發現的 checkbox 清單（格式同報告，方便使用者快速瀏覽）
2. 詢問使用者：

   > 是否需要我協助排程修改上述問題？我可以逐一協助修復，每修完一項就在報告中更新對應的 checkbox。

3. 若使用者同意，逐一處理各項問題：
   - 說明這項問題的修復方向，等使用者確認後執行修改
   - 修改完成後，用 Edit 工具將報告中對應的 `- [ ]` 改為 `- [x]`
   - 繼續處理下一項

4. 每次修完都即時更新 checkbox，讓使用者能從報告追蹤整體進度

5. 若 MEDIUM 或 LOW 的問題也需要協助，在 CRITICAL/HIGH 都處理完後再詢問

### 9. Report summary

所有協助修復的問題處理完（或使用者不需要協助時），回報：

- 產生的報告路徑
- 各等級發現數量（修復前 vs. 修復後）
- 仍未修復的 CRITICAL/HIGH 項目（若有）
- 未涵蓋的範圍或建議的後續審查方向
- 提醒：修復後應執行 `/commit` 將程式碼變更與更新後的報告一起 commit
