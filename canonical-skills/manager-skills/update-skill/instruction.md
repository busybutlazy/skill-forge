...
- 修改 `canonical-skills/manager-skills/` 下的管理者技能（此 skill 僅適用於 `regular-skills/`）
- 安裝或移除其他專案的 skills
- 建立全新的 skill（改用 `create-skill`）
- 把 canonical 狀態同步到本 repo 的 `.agents/` 或 `.claude/`（改用 `install-manager-skill`）
...
手動同步調整（直接編輯 `package.json`）：

- `identity.version`
- `identity.updated_at`
- 必要時調整 `identity.description`、`identity.tags`
...
**不要手動編輯** `manifest.json` 或 `package.json.integrity.package_sha256`。這些 hash 欄位由 `finalize-skill` 在 Docker 環境中重新計算，手動修改會與實際內容不同步。
